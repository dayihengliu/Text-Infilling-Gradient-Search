...
def _beam_search_step(time, logits, next_cell_state, beam_state, batch_size,
                      beam_width, end_token, length_penalty_weight, gamma, vocab_size):
  ...
  # Calculate indices for each beam
  scores = tf.squeeze(scores)
  score_list = tf.split(scores, beam_width, 0)
  diverse_score_list = []
  for score in score_list:
      score = tf.squeeze(score)
      values = tf.constant([_ for _ in range(vocab_size)])
      _, index = nn_ops.top_k(score, k=array_ops.shape(scores)[-1], sorted=False)
      index = tf.expand_dims(index, -1)
      shape = tf.constant([vocab_size])
      indices = tf.scatter_nd(index, values, shape)
      score = score - tf.multiply(tf.cast(indices, tf.float32), gamma)
      diverse_score_list.append(tf.expand_dims(score, 0))

  diverse_score = tf.concat(diverse_score_list, 0)
  scores_diverse = tf.expand_dims(diverse_score, 0)
  
  time = ops.convert_to_tensor(time, name="time")
  # During the first time step we only consider the initial beam
  scores_shape = array_ops.shape(scores_diverse)
  scores_flat = array_ops.reshape(scores_diverse, [batch_size, -1])
